...
static void demo_string(void)
{
  ...
  printf("\n--- KMP all-positions in \"dynamic_programming\" for \"am\" ---\n");
  {
    ...
    printf("  text: \"%s\"\n  pattern \"am\" found %d time(s) at index:",
           target, cnt);
    for (int k = 0; k < cnt; k++)
      printf(" %d", pos[k]);
    printf("\n");
...
static void demo_all_numeric_types(void)
{
  ...
  }
  {
    ...
    unsigned long long vals[] = {10000000000ULL, 20000000000ULL, 5000000000ULL,
                                 30000000000ULL, 15000000000ULL};
    for (size_t i = 0; i < 5; i++)
      rb_insert(t, &vals[i]);
    printf("unsigned llong: ");
    rb_print(t);
    printf("  valid: %s\n", rb_verify(t) ? "yes" : "NO");
    rb_destroy(t);
  }
